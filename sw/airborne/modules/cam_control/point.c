  ...
  if (cam_mode == CAM_MODE_STABILIZED || cam_mode == CAM_MODE_RC || cam_mode == CAM_MODE_WP_TARGET
    ...
    if (cam_mode == CAM_MODE_STABILIZED || cam_mode == CAM_MODE_RC) {
      ...
      if ((*fbw_state).channels[RADIO_TILT] >= 0) {
        cam_theta = (float)RadOfDeg(CAM_TILT_NEUTRAL) + (((float)(*fbw_state).channels[RADIO_TILT] /
                    (float)MAX_PPRZ) *
      ...
      if ((*fbw_state).channels[RADIO_PITCH] >= 0) {
        cam_theta = (float)RadOfDeg(CAM_TILT_NEUTRAL) + (((float)(*fbw_state).channels[RADIO_PITCH] /
                    (float)MAX_PPRZ) *
                    (float)(RadOfDeg(CAM_TILT_MAX - CAM_TILT_NEUTRAL)));
...
#error RADIO_TILT or RADIO_PITCH not defined.
      ...
      if ((*fbw_state).channels[RADIO_ROLL] >= 0) {
        cam_phi = RadOfDeg(CAM_PAN_NEUTRAL) + (((float)(*fbw_state).channels[RADIO_ROLL] /
                                                (float)MAX_PPRZ) *
                                               RadOfDeg(CAM_PAN_MAX - CAM_PAN_NEUTRAL));
...
#if defined(RADIO_PAN)
      cam_phi = RadOfDeg(CAM_PAN_MIN) + (RadOfDeg(CAM_PAN_MAX - CAM_PAN_MIN) * ((float)(
                                           *fbw_state).channels[RADIO_PAN] / (float)MAX_PPRZ));
#elif defined(RADIO_ROLL)
      cam_phi = RadOfDeg(CAM_PAN_MIN) + (RadOfDeg(CAM_PAN_MAX - CAM_PAN_MIN) * ((float)(
                                           *fbw_state).channels[RADIO_ROLL] / (float)MAX_PPRZ));
#else
#error RADIO_PAN or RADIO_ROLL not defined.
#endif
...
//cam_theta = RadOfDeg(70);     //  LOOK 45 DEGREES DOWN, 0 IS STRAIGHT DOWN 90 IS STRAIGHT IN FRONT
      ...
      if (cam_theta > RadOfDeg(80)
          && cam_mode == CAM_MODE_RC) { // Not much to see after 80 degrees of tilt so stop tracking.
        *fPan = cam_phi;
        *fTilt = cam_theta;
      ...
      float heading_radians = RadOfDeg(90) -
                              fYawAngle; //Convert the gps heading (radians) to standard mathematical notation.
...
#ifdef SHOW_CAM_COORDINATES
      cam_point_distance_from_home = distance_correction * (uint16_t) fabs(((uint16_t)(sqrt((
                                       fObjectNorth * fObjectNorth) + (fObjectEast * fObjectEast)))) -
