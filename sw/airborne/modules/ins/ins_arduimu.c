...
void ArduIMU_periodicGPS(void)
{
  ...
  if (messageNr == 0) {
    ...
    ardu_gps_trans.buf[0] =
      1;      //message Nr = 1 --> ground course, ecefVZ, numSV, Fix, flags, fix, flags
...
void IMU_Daten_verarbeiten(void)
{
  ...
  RunOnceEvery(15, DOWNLINK_SEND_AHRS_EULER(DefaultChannel, DefaultDevice, &att->phi, &att->theta,
               &att->psi));
}
