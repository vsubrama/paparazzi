...
void kalmanEstimation(TypeKalman* k, float accVert)
{
  ...
  // Calcul de X
  // X(k+1) = Ad*X(k) + Bd*U(k)
  k->X[0] = k->Ad[0][0] * k->X[0] + k->Ad[0][1] * k->X[1] + k->Ad[0][2] * k->X[2] + k->Bd[0] *
            accVert;
  k->X[1] = k->Ad[1][0] * k->X[0] + k->Ad[1][1] * k->X[1] + k->Ad[1][2] * k->X[2] + k->Bd[1] *
            accVert;
  k->X[2] = k->Ad[2][0] * k->X[0] + k->Ad[2][1] * k->X[1] + k->Ad[2][2] * k->X[2] + k->Bd[2] *
            accVert;
