...
void nav_circle_XY(float x, float y, float radius)
{
  ...
  if (nav_in_circle) {
    ...
    if (trigo_diff >
        0) { // do not rewind if the change in angle is in the opposite sense than nav_radius
  ...
  circle_bank =
    (dist2_center > Square(abs_radius + dist_carrot)
     || dist2_center < Square(abs_radius - dist_carrot)) ?
    0 :
    atanf((*stateGetHorizontalSpeedNorm_f()) * (*stateGetHorizontalSpeedNorm_f()) /
          (NAV_GRAVITY * radius));
...
static inline bool_t compute_TOD(uint8_t _af, uint8_t _td, uint8_t _tod, float glide_airspeed,
                                 float glide_vspeed)
{
...
void fly_to_xy(float x, float y)
{
  ...
  if (nav_mode == NAV_MODE_COURSE) {
    ...
    h_ctl_roll_setpoint = atanf(2 * speed * speed * s * h_ctl_course_pgain /
...
void nav_route_xy(float last_wp_x, float last_wp_y, float wp_x, float wp_y)
{
  ...
  nav_leg_progress = ((stateGetPositionEnu_f()->x - last_wp_x) * leg_x +
                      (stateGetPositionEnu_f()->y - last_wp_y) * leg_y) / leg2;
...
void nav_oval(uint8_t p1, uint8_t p2, float radius)
{
  ...
  switch (oval_status) {
    ...
    case OR21:
      ...
      if (nav_approaching_xy(waypoints[p1].x, waypoints[p1].y, waypoints[p2].x, waypoints[p2].y,
                             CARROT)) {
        oval_status = OC1;
        InitStage();
        LINE_STOP_FUNCTION;
