...
void imu_impl_init(void)
{
  ...
  imu_navgo.itg.config.smplrt_div =
    NAVGO_GYRO_SMPLRT_DIV;  // 500Hz sample rate since internal is 1kHz
...
void imu_periodic(void)
{
  ...
  RunOnceEvery((PERIODIC_FREQUENCY / (2 * 3200 >> (0xf - NAVGO_ACCEL_RATE))),
               adxl345_i2c_periodic(&imu_navgo.adxl));
...
void imu_navgo_event(void)
{
  ...
  if (imu_navgo.itg.data_available) {
    RATES_ASSIGN(imu.gyro_unscaled, -imu_navgo.itg.data.rates.q, imu_navgo.itg.data.rates.p,
                 imu_navgo.itg.data.rates.r);
#if NAVGO_USE_MEDIAN_FILTER
  ...
  if (imu_navgo.adxl.data_available) {
    VECT3_ASSIGN(imu.accel_unscaled, imu_navgo.adxl.data.vect.y, -imu_navgo.adxl.data.vect.x,
                 imu_navgo.adxl.data.vect.z);
#if NAVGO_USE_MEDIAN_FILTER
