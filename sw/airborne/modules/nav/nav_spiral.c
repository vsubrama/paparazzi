...
bool_t nav_spiral_setup(uint8_t center_wp, uint8_t edge_wp, float radius_start, float radius_inc,
                        float segments)
{
...
bool_t nav_spiral_run(void)
{
  ...
  switch (nav_spiral.status) {
    case SpiralOutside:
      ...
      nav_route_xy(nav_spiral.fly_from.x, nav_spiral.fly_from.y, nav_spiral.center.x,
                   nav_spiral.center.y);
      // center reached?
      if (nav_approaching_xy(nav_spiral.center.x, nav_spiral.center.y, nav_spiral.fly_from.x,
                             nav_spiral.fly_from.y, 0)) {
        // nadir image
#ifdef DIGITAL_CAM
