...
static void put_bytes(void* impl, enum DownlinkDataType data_type,
                      uint8_t len __attribute__((unused)), const void* bytes)
{
  ...
  while (length++ <= onboard->array_length) {
    if (onboard->array_length > 0 && length > 1) {
      onboard->buffer_idx += snprintf(onboard->buffer + onboard->buffer_idx,
                                      ONBOARD_BUFFER_LEN - onboard->buffer_idx, ",");
    } else {
      onboard->buffer_idx += snprintf(onboard->buffer + onboard->buffer_idx,
                                      ONBOARD_BUFFER_LEN - onboard->buffer_idx, " ");
    }
    switch (data_type) {
      ...
      case DL_TYPE_FLOAT:
        onboard->buffer_idx += snprintf(onboard->buffer + onboard->buffer_idx,
                                        ONBOARD_BUFFER_LEN - onboard->buffer_idx, "%#f", *(const float*)bytes);
      ...
      case DL_TYPE_TIMESTAMP:
        onboard->buffer_idx += snprintf(onboard->buffer + onboard->buffer_idx,
                                        ONBOARD_BUFFER_LEN - onboard->buffer_idx, "%u.%04u", (*(const uint32_t*)bytes) / TIMESTAMP_SCALE,
                                        (*(const uint32_t*)bytes) % TIMESTAMP_SCALE);
        bytes = (const uint32_t*) bytes + 4;
        break;
                          ...
                          uint8_t payload_len __attribute__((unused)))
{
  ...
  onboard->buffer_idx += snprintf(onboard->buffer + onboard->buffer_idx,
                                  ONBOARD_BUFFER_LEN - onboard->buffer_idx, " %s", name);
}
...
static void count_bytes(void* onboard __attribute__((unused)),
                        uint8_t bytes __attribute__((unused)))
{

}

static int check_free_space(void* onboard __attribute__((unused)),
                            uint8_t bytes __attribute__((unused)))
{
  return TRUE;
}
