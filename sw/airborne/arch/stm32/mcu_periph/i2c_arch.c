    ...
    struct i2c_transaction* trans)
{
  ...
  else if ((BIT_X_IS_SET_IN_REG(I2C_SR1_TxE, SR1))
           && (BIT_X_IS_SET_IN_REG(I2C_CR2_ITBUFEN, I2C_CR2(i2c)))) {
    // Send the next byte
...
// Transfer Sequence Diagram for Master Receiver for N>2
static inline enum STMI2CSubTransactionStatus stmi2c_readmany(uint32_t i2c,
    struct i2c_periph* periph, struct i2c_transaction* trans)
{
  uint16_t SR1 = I2C_SR1(i2c);
...
static inline void i2c_error(struct i2c_periph* periph)
{
  ...
  if ((I2C_SR1((uint32_t)periph->reg_addr) & I2C_SR1_BERR) !=
      0) {     /* Misplaced Start or Stop condition */
