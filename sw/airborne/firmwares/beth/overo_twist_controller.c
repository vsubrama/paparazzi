...
// controller.omega_tilt_ref = RadOfDeg(600);
  ...
  printf("Vm=%f VM=%f satval1=%f satval2=%f c=%f\n", controller.Vm, controller.VM, controller.satval1,
         controller.satval2, controller.c);
}
...
void control_run(void)
{
  ...
  controller.tilt_ddot_ref = -2 * controller.omega_tilt_ref * controller.xi_ref *
                             controller.tilt_dot_ref
  ...
  controller.elevation_dot_ref = controller.elevation_dot_ref + controller.elevation_ddot_ref *
                                 dt_ctl;
  controller.elevation_ddot_ref = -2 * controller.omega_elevation_ref * controller.xi_ref *
  ...
  controller.azimuth_ddot_ref = -2 * controller.omega_azimuth_ref * controller.xi_ref *
  ...
  controller.cmd_thrust_fb = -controller.mass * (2 * controller.xi_cl * controller.omega_cl *
                             err_elevation_dot) -
  ...
  controller.cmd_azimuth_fb = controller.one_over_J * (2 * controller.xi_cl * controller.omega_cl *
                              err_azimuth_dot) +
...
float get_U_twt()
{
  ...
  controller.S[1] = (float)(controller.c * (y[1] - yd[1]) + estimator.tilt_dot -
                            controller.tilt_dot_ref);
...
float get_U_twt2()
{
  ...
  //controller.S[1] = (double)( ( (1+controller.c) * (y[1]-yd[1]) - (y[0]-yd[0]) )  ) ;
  //controller.S[1] = (double)( ( (1+controller.c) * (y[1]-yd[1]) - estimator.tilt_dot ) * 0.8 ) ;
  S2[1] = (float)(controller.c * (y2[1] - yd2[1]) + estimator.elevation_dot -
                  controller.elevation_dot_ref);
  S2_dot = (S2[1] - S2[0]);
  /*************************************/
