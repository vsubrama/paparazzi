...
static inline void superbitrf_radio_to_channels(uint8_t* data, uint8_t nb_channels, bool_t is_11bit,
    int16_t* channels);
...
void superbitrf_init(void)
{
  ...
  cyrf6936_init(&superbitrf.cyrf6936, &(SUPERBITRF_SPI_DEV), 2, SUPERBITRF_RST_PORT,
                SUPERBITRF_RST_PIN);
...
void superbitrf_set_mfg_id(uint32_t id)
{
  ...
  superbitrf.sop_col = (superbitrf.bind_mfg_id[0] + superbitrf.bind_mfg_id[1] +
                        superbitrf.bind_mfg_id[2] + 2) & 0x07;
...
void superbitrf_event(void)
{
  ...
  if (superbitrf.status != SUPERBITRF_UNINIT) {
    ...
    if (superbitrf.cyrf6936.has_irq && (superbitrf.cyrf6936.rx_irq_status & CYRF_RXC_IRQ)) {
      ...
      superbitrf_receive_packet_cb((superbitrf.cyrf6936.rx_irq_status & CYRF_RXE_IRQ),
                                   superbitrf.cyrf6936.rx_status, superbitrf.cyrf6936.rx_packet);
      superbitrf.rx_packet_count++;
    ...
    case SUPERBITRF_TRANSFER:
      ...
      switch (superbitrf.state) {
        ...
        default:
          // Set the timer
          if (superbitrf.crc_seed != ((superbitrf.bind_mfg_id[0] << 8) + superbitrf.bind_mfg_id[1])) {
            superbitrf.timer = (superbitrf.timer - SUPERBITRF_DATARECV_TIME + SUPERBITRF_RECV_TIME) %
                               0xFFFFFFFF;
          } else {
            superbitrf.timer = (superbitrf.timer - SUPERBITRF_DATARECV_TIME + SUPERBITRF_RECV_SHORT_TIME) %
                               0xFFFFFFFF;
          }
          if (superbitrf.timer < get_sys_time_usec()) {
            superbitrf.timer_overflow = TRUE;
...
static inline void superbitrf_receive_packet_cb(bool_t error, uint8_t status, uint8_t packet[])
{
  ...
  switch (superbitrf.status) {
    ...
    case SUPERBITRF_BINDING:
      ...
      if (packet[0] != packet[4] || packet[1] != packet[5] || packet[2] != packet[6]
          || packet[3] != packet[7]) {
    ...
    case SUPERBITRF_SYNCING_A:
      ...
      if ((IS_DSM2(superbitrf.protocol) || SUPERBITRF_FORCE_DSM2) &&
          (packet[0] != (~superbitrf.bind_mfg_id[2] & 0xFF)
           || (packet[1] != (~superbitrf.bind_mfg_id[3] & 0xFF) &&
      ...
      if (packet[1] != (~superbitrf.bind_mfg_id[3] & 0xFF)
          && packet[1] != (superbitrf.bind_mfg_id[3] & 0xFF)) {
        superbitrf.uplink_count++;
    ...
    case SUPERBITRF_SYNCING_B:
      ...
      if ((IS_DSM2(superbitrf.protocol) || SUPERBITRF_FORCE_DSM2) &&
          (packet[0] != (~superbitrf.bind_mfg_id[2] & 0xFF)
           || (packet[1] != (~superbitrf.bind_mfg_id[3] & 0xFF) &&
               packet[1] != (~superbitrf.bind_mfg_id[3] & 0xFF) + 1
               && packet[1] != (~superbitrf.bind_mfg_id[3] & 0xFF) + 2))) {
        // Start receiving TODO: Fix nicely
        cyrf6936_multi_write(&superbitrf.cyrf6936, cyrf_start_receive, 2);
        break;
      }
    ...
    case SUPERBITRF_TRANSFER:
      ...
      if (packet[1] == (~superbitrf.bind_mfg_id[3] & 0xFF)
        ...
        // Parse the packet
        superbitrf_radio_to_channels(&packet[2], superbitrf.num_channels, superbitrf.resolution,
                                     superbitrf.rc_values);
...
static inline void superbitrf_gen_dsmx_channels(void)
{
  ...
  while (idx < 23) {
    ...
    uint8_t next_ch = ((id_tmp >> 8) % 0x49) +
                      3;       // Use least-significant byte and must be larger than 3
