...
static void put_named_uint8_t(struct udp_transport* udp, char* name __attribute__((unused)),
                              const uint8_t byte)
{
  put_uint8_t(udp, byte);
}

static void put_bytes(void* impl, enum DownlinkDataType data_type __attribute__((unused)),
...
static int check_free_space(void* udp __attribute__((unused)),
