...
void motor_mixing_run_spinup(uint32_t counter, uint32_t max_counter)
{
  ...
  for (i = 0; i < MOTOR_MIXING_NB_MOTOR; i++) {
    ...
    if (counter > i * max_counter / (MOTOR_MIXING_NB_MOTOR + MOTOR_MIXING_STARTUP_DELAY)) {
      if (counter > MOTOR_MIXING_NB_MOTOR * max_counter / (MOTOR_MIXING_NB_MOTOR +
          MOTOR_MIXING_STARTUP_DELAY)) {
        motor_mixing.commands[i] = MOTOR_MIXING_MIN_MOTOR_STARTUP + (MOTOR_MIXING_MIN_MOTOR -
                                   MOTOR_MIXING_MIN_MOTOR_STARTUP) * counter / max_counter;
      } else {
