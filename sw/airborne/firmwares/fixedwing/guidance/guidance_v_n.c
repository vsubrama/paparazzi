...
static inline void v_ctl_set_airspeed(void)
{
  ...
  if (v_ctl_auto_airspeed_pitch_igain > 0.) {
    ...
    BoundAbs(v_ctl_auto_airspeed_pitch_sum_err,
             V_CTL_AUTO_AIRSPEED_PITCH_MAX_SUM_ERR / v_ctl_auto_airspeed_pitch_igain);
  }
...
static inline void v_ctl_set_groundspeed(void)
{
  ...
  v_ctl_auto_airspeed_setpoint = err_groundspeed * v_ctl_auto_groundspeed_pgain +
