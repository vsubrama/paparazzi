...
class CommChannels:
	...
	def initConnection(self):
		...
		print "LOG : CommChannel's outputConn is : ", self.outputConn
		...
		while(len(self.inputConn)<self.inputNum):
			print "LOG : len(self.inputConn) is : ", len(self.inputConn)
			conn, addr = localSocket.accept()#new connected socket

			print "LOG : got connection from addr : ", addr
			f = conn.makefile()
			line = f.readline()
			line = line[0:-1]#get rid of linebreaker
			print "LOG : got line from the addr : ", line
			if line == "TEST":
				print "ACK"
			else:
				self.inputConn[line] = conn
				print "recieve:" + line
		print "LOG : CommChannel's inputConn is : ", self.inputConn
...
if __name__ == '__main__':
	...
	# write the sivybus variable to the log file as well
	print 'LOG : sivybus is : ', sivybus
	
	if sivybus != "" :
		sechoivybus = sivybus
		print 'LOG : in if condition sechoivybus is : ', sechoivybus
	elif os.environ.has_key("IVYBUS"):
		sechoivybus = os.environ["IVYBUS"]
		print 'LOG : in elif condition sechoivybus is : ', sechoivybus
	else:
		sechoivybus = "ivydefault"
		print 'LOG : in else condition sechoivybus : ', sechoivybus
