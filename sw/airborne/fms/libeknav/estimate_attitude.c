...
struct DoubleMat44 square_skaled(struct DoubleMat44 A)
{
  ...
  for (row = 0; row < 4; row++) {
    for (col = 0; col < 4; col++) {
      M4(A2, row, col) = M4(A, row, 0) * M4(A, 0, col) + M4(A, row, 1) * M4(A, 1, col) + M4(A, row,
                         2) * M4(A, 2, col) + M4(A, row, 3) * M4(A, 3, col);
 ...
 *
 */
DoubleVect4 dominant_Eigenvector(struct DoubleMat44 A, unsigned int maximum_iterations,
                                 double precision, struct DoubleMat44 sigma_A, DoubleVect4* sigma_x)
{
  unsigned int  k;
  DoubleVect4 x_k,
              x_kp1;
 ...
 *
 */
struct DoubleQuat estimated_attitude(struct DoubleMat33 B, unsigned int maximum_iterations,
                                     double precision, struct DoubleMat33 sigma_B, struct DoubleQuat* sigma_q)
{
  double      traceB,
              z1, z2, z3;
