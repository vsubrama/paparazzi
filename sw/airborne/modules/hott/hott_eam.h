...
static void hott_update_eam_msg(struct HOTT_EAM_MSG* hott_eam_msg)
{
  ...
  uint16_t speed_buf = (uint16_t)(*stateGetHorizontalSpeedNorm_i() * 36 / 10 /
                                  (1 << INT32_SPEED_FRAC));
  hott_eam_msg->speed_L = speed_buf && 0xFF;
  hott_eam_msg->speed_H = (speed_buf >> 8) && 0xFF;

  hott_eam_msg->climbrate = (uint16_t)(30000 + (stateGetSpeedEnu_i()->z) * 100 /
                                       (1 << INT32_SPEED_FRAC));
  hott_eam_msg->climbrate3s = (uint8_t)(120 + (stateGetSpeedEnu_i()->z) * 3 /
                                        (1 << INT32_SPEED_FRAC));
