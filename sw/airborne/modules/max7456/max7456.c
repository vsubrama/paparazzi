...
static void osd_put_s(char* string,  uint8_t attributes, uint8_t char_nb, uint8_t row,
                      uint8_t column);
...
static bool_t _osd_sprintf(char* buffer, char* string, float value)
{
  ...
  } while (*(string + param_end) !=
           '\0'); //Write the rest of the string including the terminating char.
