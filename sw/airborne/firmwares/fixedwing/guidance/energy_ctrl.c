...
// Ground speed control loop (input: groundspeed error, output: airspeed controlled)
  ...
  v_ctl_auto_airspeed_controlled = (err_groundspeed + v_ctl_auto_groundspeed_sum_err *
  ...
  float vdot = ACCEL_FLOAT_OF_BFP(accel_meas_body.x) / 9.81f - sinf(
  ...
  float gamma_err  = (v_ctl_climb_setpoint - stateGetSpeedEnu_f()->z) /
  ...
  /* pitch pre-command */
  if (launch && (v_ctl_mode >= V_CTL_MODE_AUTO_CLIMB)) {
    v_ctl_auto_throttle_nominal_cruise_pitch +=  v_ctl_auto_pitch_of_airspeed_igain *
        (-speed_error) * dt_attidude
  ...
  float v_ctl_pitch_of_vz =
    + (v_ctl_climb_setpoint /*+ d_err * v_ctl_auto_throttle_pitch_of_vz_dgain*/) *
