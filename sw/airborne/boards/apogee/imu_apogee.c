...
bool_t configure_baro_slave(Mpu60x0ConfigSet mpu_set __attribute__((unused)),
...
void imu_apogee_event(void)
{
  ...
  if (imu_apogee.mpu.data_available) {
    RATES_ASSIGN(imu.gyro_unscaled, imu_apogee.mpu.data_rates.rates.p,
                 -imu_apogee.mpu.data_rates.rates.q, -imu_apogee.mpu.data_rates.rates.r);
    VECT3_ASSIGN(imu.accel_unscaled, imu_apogee.mpu.data_accel.vect.x,
                 -imu_apogee.mpu.data_accel.vect.y, -imu_apogee.mpu.data_accel.vect.z);
