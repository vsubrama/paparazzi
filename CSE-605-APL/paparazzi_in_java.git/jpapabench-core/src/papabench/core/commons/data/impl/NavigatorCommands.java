...
public abstract class NavigatorCommands {
	...
	/**
	 * Decide if the UAV is approaching the current waypoint. Computes
	 * dist2_to_wp and compare it to square carrot. Return true if it is
	 * smaller. Else computes by scalar products if uav has not gone past
	 * waypoint. Return true if it is the case.
	 */
	...
	protected final boolean navApproachingFrom(Position3D toWP,
		...
		float minDist = approachingTime
				* estimator().getHorizontalSpeed().module;

		// System.out
		// .println("navApproachingFrom[" + pwX + "," + pwY
		// + "] distance to WP " + dist2ToWP + " minDistance "
		// + (minDist * minDist) + "\n approachingTime "
		// + approachingTime + " horizontalspeed "
		// + estimator().getHorizontalSpeed().module);

		if (dist2ToWP < minDist * minDist) {
			System.out.println("exiting with true!");
			return true;
		}

		float scalarProduct = MathUtils.scalarProduct(toWP, fromWP, estimator()
				.getPosition());

		// System.out.println("scalarProduct" + scalarProduct);
	...
	protected final void navRouteXY(Position3D startWP, Position3D endWP) {
		...
		float navLegProgress = ((estimator().getPosition().x - startWP.x)
				* legX + (estimator().getPosition().y - startWP.y) * legY)
				/ leg2;
	...
	// WARNING - this method is implemented according to PapaBench code (it is
	// not compatible with paparazzi)
	protected final void flyToXY(float x, float y) {

		navigator().setDesiredPosition(x, y);
		navigator().setDesiredCourse(
				(float) (Math.PI / 2 - Math.atan2(y
						- estimator().getPosition().y, x
	...
	protected final void routeTo(Position3D fromWP, Position3D toWP) {
		...
		float leg2 = Math.max(legX * legX + legY * legY, 1f);
		float alpha = ((estimator().getPosition().x - fromWP.x) * legX + (estimator()
