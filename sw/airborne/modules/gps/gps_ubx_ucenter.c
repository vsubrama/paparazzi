...
static inline void gps_ubx_ucenter_config_nav(void)
{
  ...
  if (gps_ubx_ucenter.sw_ver_h < 5 && gps_ubx_ucenter.hw_ver_h < 6) {
    UbxSend_CFG_NAV(NAV_DYN_AIRBORNE_2G, 3, 16, 24, 20, 5, 0, 0x3C, 0x3C, 0x14, 0x03E8 , 0x0000, 0x0,
                    0x17, 0x00FA, 0x00FA, 0x0064, 0x012C, 0x000F, 0x00, 0x00);
  } else {
    UbxSend_CFG_NAV5(NAV5_MASK, GPS_UBX_NAV5_DYNAMICS, NAV5_3D_ONLY, IGNORED, IGNORED,
                     NAV5_DEFAULT_MIN_ELEV, RESERVED,
...
static inline void gps_ubx_ucenter_config_port(void)
{
  switch (gps_ubx_ucenter.port_id) {
    ...
    case GPS_PORT_DDC:
#ifdef GPS_I2C
      UbxSend_CFG_PRT(gps_ubx_ucenter.port_id, 0x0, 0x0, GPS_I2C_SLAVE_ADDR, 0x0, UBX_PROTO_MASK,
    ...
    case GPS_PORT_UART1:
    case GPS_PORT_UART2:
      UbxSend_CFG_PRT(gps_ubx_ucenter.port_id, 0x0, 0x0, UBX_UART_MODE_MASK, UART_SPEED(UBX_GPS_BAUD),
                      UBX_PROTO_MASK, UBX_PROTO_MASK, 0x0, 0x0);
      break;
      // USB Interface
    case GPS_PORT_USB:
      UbxSend_CFG_PRT(gps_ubx_ucenter.port_id, 0x0, 0x0, 0x0, 0x0, UBX_PROTO_MASK, UBX_PROTO_MASK, 0x0,
                      0x0);
      break;
    case GPS_PORT_SPI:
...
static inline void gps_ubx_ucenter_config_sbas(void)
{
  ...
  UbxSend_CFG_SBAS(GPS_SBAS_ENABLED, GPS_SBAS_RANGING | GPS_SBAS_CORRECTIONS | GPS_SBAS_INTEGRITY,
...
static bool_t gps_ubx_ucenter_configure(uint8_t nr)
{
  ...
  switch (nr) {
    ...
    case 18:
      ...
      DOWNLINK_SEND_DEBUG(DefaultChannel, DefaultDevice, GPS_UBX_UCENTER_CONFIG_STEPS,
                          gps_ubx_ucenter.replies);
      for (int i = 0; i < GPS_UBX_UCENTER_CONFIG_STEPS; i++) {
