 ...
 * @param dss data size
 */
__attribute__((always_inline)) static inline void SpiSetDataSize(struct spi_periph* p,
    enum SPIDataSizeSelect dss)
{
  switch (dss) {
    default:
      case SPIDss8bit:
          ((sspRegs_t*)(p->reg_addr))->cr0 = (((sspRegs_t*)(p->reg_addr))->cr0 & ~(0xF << DSS)) |
                                             (DSS_VAL8 << DSS);
      break;
    case SPIDss16bit:
      ((sspRegs_t*)(p->reg_addr))->cr0 = (((sspRegs_t*)(p->reg_addr))->cr0 & ~(0xF << DSS)) |
...
__attribute__((always_inline)) static inline void SpiTransmit(struct spi_periph* p,
...
__attribute__((always_inline)) static inline void SpiReceive(struct spi_periph* p,
    struct spi_transaction* t)
{
...
__attribute__((always_inline)) static inline void SpiInitBuf(struct spi_periph* p,
    struct spi_transaction* t)
{
  p->rx_idx_buf = 0;
  p->tx_idx_buf = 0;
...
__attribute__((always_inline)) static inline void SpiStart(struct spi_periph* p,
    struct spi_transaction* t)
{
  p->status = SPIRunning;
...
__attribute__((always_inline)) static inline void SpiEndOfTransaction(struct spi_periph* p,
    struct spi_transaction* t)
{
...
__attribute__((always_inline)) static inline void SpiSlaveStart(struct spi_periph* p,
    struct spi_transaction* t)
{
  p->status = SPIRunning;
  t->status = SPITransRunning;
