...
{
  data[buflen - 1] = (unsigned char)(UpdateCRC(0xFFFFFFFF , data ,
  ...
  switch (pstData->m_State) {
    ...
    case CHIMU_STATE_MACHINE_LEN:  // Get chars to read
      if (btData <= CHIMU_RX_BUFFERSIZE) {
        ...
        pstData->m_State =
          CHIMU_STATE_MACHINE_START; //Length byte exceeds buffer.  Signal a fail and restart
...
unsigned char CHIMU_ProcessMessage(unsigned char* pMsgID __attribute__((unused)),
                                   unsigned char* pPayloadData, CHIMU_PARSER_DATA* pstData)
{
