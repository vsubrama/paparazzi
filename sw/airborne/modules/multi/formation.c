...
int add_slot(uint8_t _id, float slot_e, float slot_n, float slot_a)
{
  ...
  DOWNLINK_SEND_FORMATION_SLOT_TM(DefaultChannel, DefaultDevice, &_id, &form_mode, &slot_e, &slot_n,
                                  &slot_a);
...
int formation_flight(void)
{
  ...
  SetAcInfo(AC_ID, stateGetPositionEnu_f()->x, stateGetPositionEnu_f()->y,
            (*stateGetHorizontalSpeedDir_f()), stateGetPositionUtm_f()->alt, (*stateGetHorizontalSpeedNorm_f()),
            stateGetSpeedEnu_f()->z, gps.tow);
  ...
  for (i = 0; i < NB_ACS; ++i) {
    ...
    if (formation[i].status == ACTIVE && fabs(stateGetPositionUtm_f()->alt - ac->alt) < form_prox
        && ac->alt > 0) {
      ...
      form_a += (ac->alt - stateGetPositionUtm_f()->alt) - (formation[i].alt -
                formation[the_acs_id[AC_ID]].alt);
