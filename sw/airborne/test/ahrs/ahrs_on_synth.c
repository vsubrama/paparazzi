...
static void traj_step_phi_2nd_order_update(void)
{
  ...
  if (aos.time > 15) {
    ...
    RATES_ASSIGN(raccel, -2.*xi * omega * aos.imu_rates.p - omega * omega *
                 (aos.ltp_to_imu_euler.phi - RadOfDeg(5)), 0., 0.);
...
static void traj_coordinated_circle_update(void)
{
  ...
  if (aos.time > 2.*M_PI / omega) {
    VECT3_ASSIGN(aos.ltp_pos,                R * cos(omega * aos.time),
                 R * sin(omega * aos.time), 0.);
    VECT3_ASSIGN(aos.ltp_vel,         -omega * R * sin(omega * aos.time),
                 omega * R * cos(omega * aos.time), 0.);
    VECT3_ASSIGN(aos.ltp_accel, -omega * omega * R * cos(omega * aos.time),
                 -omega * omega * R * sin(omega * aos.time), 0.);
