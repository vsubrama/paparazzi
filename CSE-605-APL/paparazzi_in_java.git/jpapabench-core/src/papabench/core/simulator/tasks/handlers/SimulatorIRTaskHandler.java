...
public class SimulatorIRTaskHandler implements Runnable {
	...
	public void run() {
//		if( !isExe ){
//			System.out.println("start: Thread_" + SimulatorIRTaskConf.PRIORITY+" : " + Thread.currentThread().getName());
//			isExe = true;
//		}
		//System.out.println("SimulatorIRTaskHandler start ###");
		Attitude attitude = flightModel.getState().getAttitude();
		//System.out.println("update:" + flightModel.getState().getAttitude().phi +","+ flightModel.getState().getAttitude().psi +","+flightModel.getState().getAttitude().theta);
		//LogUtils.log(this, "Att: " +attitude.toString());
	
		this.irDevice.update(this.flightModel);		
		//System.out.println("... SimulatorIRTaskHandler done ###");
	}
}
