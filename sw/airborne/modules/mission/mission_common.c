...
bool_t mission_insert(enum MissionInsertMode insert, struct _mission_element* element)
{
  ...
  switch (insert) {
    case Append:
      ...
      memcpy(&mission.elements[mission.insert_idx], element,
             sizeof(struct _mission_element)); // add element
...
bool_t __attribute__((weak)) mission_element_convert(struct _mission_element* el __attribute__((
      unused))) { return TRUE; }
