...
void humid_sht_periodic_i2c(void)
{
  switch (sht_status) {
    ...
    default:
      ...
      RunOnceEvery((4 * 30), DOWNLINK_SEND_SHT_I2C_SERIAL(DefaultChannel, DefaultDevice, &sht_serial1,
                   &sht_serial2));
      break;
  }
}
  ...
  if (sht_trans.status == I2CTransSuccess) {
    switch (sht_status) {
      ...
      case SHT2_READ_HUMID:
        ...
        if (humid_sht_crc(sht_trans.buf) == 0) {
          DOWNLINK_SEND_SHT_I2C_STATUS(DefaultChannel, DefaultDevice, &humidsht_i2c, &tempsht_i2c,
                                       &fhumidsht_i2c, &ftempsht_i2c);
        }
        break;
