 ...
 */
static bool_t intercept_two_lines(struct FloatVect2* p, struct FloatVect2 x, struct FloatVect2 y,
 ...
 */
static bool_t get_two_intersects(struct FloatVect2* x, struct FloatVect2* y, struct FloatVect2 a,
                                 struct FloatVect2 b)
{
  int i, count = 0;
  ...
  for (i = 0; i < survey.poly_count - 1; i++)
    if (intercept_two_lines(&tmp, a, b, waypoints[survey.poly_first + i].x,
                            waypoints[survey.poly_first + i].y, waypoints[survey.poly_first + i + 1].x,
                            waypoints[survey.poly_first + i + 1].y)) {
      if (count == 0) {
        *x = tmp;
        count++;
      } else {
 ...
 **/
bool_t nav_survey_polygon_setup(uint8_t first_wp, uint8_t size, float angle, float sweep_width,
                                float shot_dist, float min_rad, float altitude)
{
  int i;
...
bool_t nav_survey_polygon_run(void)
{
  ...
  if (survey.stage == SEG) {
    ...
    if (nav_approaching_xy(survey.seg_end.x, survey.seg_end.y, survey.seg_start.x, survey.seg_start.y,
                           0)) {
#ifdef DIGITAL_CAM
      dc_stop();
#endif
  ...
  //turn from stage to return
    ...
    if (nav_approaching_xy(survey.ret_end.x, survey.ret_end.y, survey.ret_start.x, survey.ret_start.y,
                           0)) {
      survey.stage = TURN2;
      nav_init_stage();
    }
    ...
    nav_circle_XY(survey.seg_center2.x, survey.seg_center2.y,
                  -(2 * survey.psa_min_rad + survey.psa_sweep_width) * 0.5);
    if (NavCourseCloseTo(survey.segment_angle)) {
      survey.stage = SEG;
      nav_init_stage();
#ifdef DIGITAL_CAM
