...
////////////////////////////////////////////////////////////////////////////////////////////////
bool_t  nav_survey_ComputeProjectionOnLine(struct point pointAf, struct point pointBf, float pos_xf,
    float pos_yf, float* normAMf, float* normBMf, float* distancefromrailf);
...
bool_t nav_survey_losange_carto_init(uint8_t wp1, uint8_t wp2,  uint8_t wp3, float distrailinit,
                                     float distplusinit)
{
  ...
  point1.x =
    waypoints[wp1].x; //the coordinates are in meter units, taken from the flight plan, in float type
  ...
  angle1213 = (180 / 3.14159) * acos((((vec12.x * vec13.x) + (vec12.y * vec13.y))) /
                                     (norm12 * norm13));//oriented angle between 12 and 13 vectors
...
bool_t nav_survey_losange_carto(void)
{
  ...
  if (survey_losange_uturn == FALSE) {
    ...
    if (railnumber == -1) {
      ...
      pointA.x = point1.x - (vec12.x / norm12) * distplus *
                 1.2; //on prend une marge plus grande pour arriver en ce point
      pointA.y = point1.y - (vec12.y / norm12) * distplus *
                 1.2; //car le virage n'est pas tres bien géré


      pointB.x = point2.x + (vec12.x / norm12) * distplus *
                 1.2; //on prend une marge plus grande pour arriver en ce point
      pointB.y = point2.y + (vec12.y / norm12) * distplus *
                 1.2; //car le virage n'est pas tres bien géré
      ...
      nav_survey_ComputeProjectionOnLine(pointA, pointB, stateGetPositionEnu_f()->x,
                                         stateGetPositionEnu_f()->y, &normAM, &normBM, &distancefromrail);

      if ((DISTXY(stateGetPositionEnu_f()->x, stateGetPositionEnu_f()->y, pointA.x,
                  pointA.y)  > 2 * DISTLIMIT)  || (normBM < (DISTXY(pointB.x, pointB.y, pointA.x, pointA.y)))) {
    ...
    if (railnumber >= 0) {
      pointA.x = (point1.x - ((vec12.x / norm12) * distplus)) + ((railnumber) *
                 (vec13.x / norm13) * distrail);
      pointA.y = (point1.y - ((vec12.y / norm12) * distplus)) + ((railnumber) *
                 (vec13.y / norm13) * distrail);

      pointB.x = (point2.x + ((vec12.x / norm12) * distplus)) + ((railnumber) *
                 (vec13.x / norm13) * distrail);
      pointB.y = (point2.y + ((vec12.y / norm12) * distplus)) + ((railnumber) *
                 (vec13.y / norm13) * distrail);
      ...
      //  PRTDEB(f,pointB.x)
      //  PRTDEB(f,pointB.y)
      ProjectionInsideLimitOfRail = nav_survey_ComputeProjectionOnLine(pointA, pointB,
                                    stateGetPositionEnu_f()->x, stateGetPositionEnu_f()->y, &normAM, &normBM, &distancefromrail);
      ...
      if (!((DISTXY(stateGetPositionEnu_f()->x, stateGetPositionEnu_f()->y, pointB.x,
                    pointB.y) < DISTLIMIT)  || (ProjectionInsideLimitOfRail
                        && (normAM > (DISTXY(pointB.x, pointB.y, pointA.x, pointA.y))))))
        //    (normBM>(DISTXY(pointB.x,pointB.y,pointA.x,pointA.y))))
      {
    ...
    if (distrail < 200) {
      ...
      if ((railnumber % 2) == 1) { //rail précédent n0, 2, 4, donc premiere direction, de wp1 vers wp2
        ...
        pointA.x = (point2.x + ((vec12.x / norm12) * distplus)) + ((railnumber - 1) *
                   (vec13.x / norm13) * distrail);
        pointA.y = (point2.y + ((vec12.y / norm12) * distplus)) + ((railnumber - 1) *
                   (vec13.y / norm13) * distrail);
        ...
        //fin du rail suivant
        pointC.x = (point1.x + ((vec12.x / norm12) * distplus)) + ((railnumber) *
                   (vec13.x / norm13) * distrail);
        pointC.y = (point1.y + ((vec12.y / norm12) * distplus)) + ((railnumber) *
                   (vec13.y / norm13) * distrail);
        ...
        if ((DISTXY(stateGetPositionEnu_f()->x, stateGetPositionEnu_f()->y, pointB.x,
                    pointB.y)  < DISTLIMIT) || (angle_between > -10 && angle_between < 10)) {
        ...
        //fin du rail précédent
        pointA.x = (point1.x - ((vec12.x / norm12) * distplus)) + ((railnumber - 1) *
                   (vec13.x / norm13) * distrail);
        pointA.y = (point1.y - ((vec12.y / norm12) * distplus)) + ((railnumber - 1) *
                   (vec13.y / norm13) * distrail);
        //début du rail suivant
        pointB.x = (point1.x - ((vec12.x / norm12) * distplus)) + ((railnumber) *
                   (vec13.x / norm13) * distrail);
        pointB.y = (point1.y - ((vec12.y / norm12) * distplus)) + ((railnumber) *
                   (vec13.y / norm13) * distrail);
        //milieu
        waypoints[0].x = (pointA.x + pointB.x) / 2;
        waypoints[0].y = (pointA.y + pointB.y) / 2;
        ...
        //fin du rail suivant
        pointC.x = (point2.x + ((vec12.x / norm12) * distplus)) + ((railnumber) *
                   (vec13.x / norm13) * distrail);
        pointC.y = (point2.y + ((vec12.y / norm12) * distplus)) + ((railnumber) *
                   (vec13.y / norm13) * distrail);
