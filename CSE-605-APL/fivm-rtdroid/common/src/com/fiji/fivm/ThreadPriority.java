...
public final class ThreadPriority {
	...
	}, RR(0x00020000), FIFO(0x00030000),
	    INVALID(0x7fff0000);

	private final int value;

	private Scheduler(int value) {
	    this.value = value;
	}
	...
	public static Scheduler fromInt(int scheduler) {
	    switch (scheduler) {
	    case 0x00000000:
		return JAVA;
	    case 0x00010000:
	...
	Scheduler sched = Scheduler.fromInt(scheduler(tpr));
	int prio = priority(tpr);
        
	if (sched == Scheduler.JAVA) {
    ...
    public static String toString(int prio) {
        String result;
        validate(prio);
	return Scheduler.fromInt(scheduler(prio)).toString()+priority(prio);
    }
    ...
    public static int fromString(String str) {
        ...
        try {
            if (str.startsWith("Java")) {
                result=Scheduler.JAVA.value()|Integer.parseInt(str.substring(4));
            } else if (str.startsWith("Normal")) {
                result=Scheduler.NORMAL.value()|Integer.parseInt(str.substring(6));
            } else if (str.startsWith("FIFO")) {
                result=Scheduler.FIFO.value()|Integer.parseInt(str.substring(4));
            } else if (str.startsWith("RR")) {
                result=Scheduler.RR.value()|Integer.parseInt(str.substring(2));
            } else {
