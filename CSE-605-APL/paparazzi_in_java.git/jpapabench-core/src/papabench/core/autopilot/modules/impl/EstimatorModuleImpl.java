	...
	public void updateIR() { // -> estimator_update_ir_estim
		if (irInitialized) {
			...
			if (dt > 0.1) {
				...
				phi = phi / dt * AirframeParametersConf.NOMINAL_AIRSPEED
				...
				if (absphi < 1.0f
						&& absphi > 0.05f
						&& (-irDevice.getIrContrast() / 2 < irDevice
								.getIrRoll() && irDevice.getIrRoll() < irDevice
								.getIrContrast() / 2)) {
	...
	public void updatePosition() {
		if (true) { // FIXME
			...
			if (LogUtils.OLD_OUTPUT)
				System.out.println(gpsDevice.getEast() + ","
						+ gpsDevice.getNorth() + gpsDevice.getAltitude() + ","
						+ gpsDevice.getSpeed() + "," + gpsDevice.getCourse()
						+ gpsDevice.getClimb());
