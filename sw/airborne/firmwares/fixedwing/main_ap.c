...
INFO_VALUE("it is recommended to configure in your airframe PERIODIC_FREQUENCY to at least ",
...
static inline uint8_t pprz_mode_update(void)
{
  if ((pprz_mode != PPRZ_MODE_HOME &&
    ...
    if (PPRZ_MODE_OF_PULSE(fbw_state->channels[RADIO_MODE]) == PPRZ_MODE_MANUAL) {
       ...
       */
      return ModeUpdate(pprz_mode, (fbw_state->channels[RADIO_AUTO_MODE] > THRESHOLD2) ? PPRZ_MODE_AUTO2 :
                        PPRZ_MODE_AUTO1);
    }
...
static inline void telecommand_task(void)
{
  ...
  if (!autopilot_flight_time) {
    if (pprz_mode == PPRZ_MODE_AUTO2
        && fbw_state->channels[RADIO_THROTTLE] > THROTTLE_THRESHOLD_TAKEOFF) {
      launch = TRUE;
    }
  }
#endif
...
void attitude_loop(void)
{
  ...
  if (pprz_mode >= PPRZ_MODE_AUTO2) {
    ...
    h_ctl_pitch_setpoint =
      v_ctl_pitch_setpoint; // Copy the pitch setpoint from the guidance to the stabilization control
