...
void baro_amsys_read_periodic(void)
{
  ...
  DOWNLINK_SEND_AMSYS_BARO(DefaultChannel, DefaultDevice, &pBaroRaw, &baro_amsys_p,
                           &baro_amsys_offset, &ref_alt_init, &baro_amsys_abs_altitude, &baro_amsys_altitude,
                           &baro_amsys_temp);
#else
  RunOnceEvery(10, DOWNLINK_SEND_AMSYS_BARO(DefaultChannel, DefaultDevice, &pBaroRaw, &baro_amsys_p,
               &baro_amsys_offset, &ref_alt_init, &baro_amsys_abs_altitude, &baro_amsys_altitude,
               &baro_amsys_temp));
#endif
}
...
void baro_amsys_read_event(void)
{
  ...
  baro_amsys_temp = (float)(tBaroRaw - TEMPERATURE_AMSYS_OFFSET_MIN) * TEMPERATURE_AMSYS_MAX /
                    (float)(TEMPERATURE_AMSYS_OFFSET_MAX - TEMPERATURE_AMSYS_OFFSET_MIN) + (float)TEMPERATURE_AMSYS_MIN;
#endif
  ...
  if (baro_amsys_valid) {
    ...
    if (!baro_amsys_offset_init) {
      ...
      baro_amsys_altitude = baro_filter * baro_old + (1 - baro_filter) * (baro_amsys_offset -
                            baro_amsys_p) * baro_scale / (1.2041 * 9.81);
